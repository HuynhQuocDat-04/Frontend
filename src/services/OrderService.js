...
export const confirmPaidOrders = async (access_token, orderIds) => {
    const res = await axiosJWT.put(`${process.env.REACT_APP_API_URL_BACKEND}/order/confirm-paid`, { orderIds }, {
        headers: { token: `Bearer ${access_token}` }
    })
    return res.data
}

...
export const orderSlide = createSlice({
  ...
  reducers: {
    ...
    increaseAmount: (state, action) => {
      ...
      // Chặn tăng số lượng nếu đã bằng tồn kho
      if(itemOrder && itemOrder.amount < itemOrder.countInstock) { 
          itemOrder.amount++; 
      }
      if(itemOrderSelected && itemOrderSelected.amount < itemOrderSelected.countInstock) { 
    ...
    decreaseAmount: (state, action) => {
      ...
      // Chặn giảm số lượng nếu đang là 1
    ...
    removeAllOrderProduct: (state, action) => {
      ...
      const itemOrdersSelected = state?.orderItemsSlected?.filter((item) => !listChecked.includes(item.product))
